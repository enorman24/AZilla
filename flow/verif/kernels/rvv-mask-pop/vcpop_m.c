/* AUTO-GENERATED by flow/verif/scripts/gen_kernels.py - directed test for vcpop.m.
 * Differential vs Spike; per-variant #V blocks. Do not hand-edit; regenerate.
 * population count of mask
 */
#include "verif.h"

static uint8_t A[2048] L2BUF;
static uint8_t B[2048] L2BUF;
static uint8_t R[4096] L2BUF;
static uint8_t D0[2048] L2BUF;
static uint8_t M[256]  L2BUF;

static void fill(void) {
    for (int i = 0; i < 2048; i++) {
        A[i]  = (uint8_t)(i * 7u + 3u);
        B[i]  = (uint8_t)((i * 13u + 131u) | 1u);   /* nonzero (safe for divide) */
        D0[i] = (uint8_t)(0xC0u + i);
    }
    for (int i = 0; i < 256; i++) M[i] = (uint8_t)(0xA5u ^ (i * 3u));
}

int main(void) {
    fill();
    VBEGIN();
    asm volatile(
    "vsetvli t0,%[vl],e8,m1,ta,ma\n"
    "vlm.v v0,(%[A])\n"
    "vcpop.m t1,v0\n"
    "sd t1,0(%[R])\n"
    :: [A]"r"(A),[R]"r"(R),[vl]"r"(64UL) : "t0","t1","memory");
    report_e64("vcpop.m m1 vl=64", R, 1);

    asm volatile(
    "vsetvli t0,%[vl],e8,m2,ta,ma\n"
    "vlm.v v0,(%[A])\n"
    "vcpop.m t1,v0\n"
    "sd t1,0(%[R])\n"
    :: [A]"r"(A),[R]"r"(R),[vl]"r"(64UL) : "t0","t1","memory");
    report_e64("vcpop.m m2 vl=64", R, 1);

    asm volatile(
    "vsetvli t0,%[vl],e8,m8,ta,ma\n"
    "vlm.v v0,(%[A])\n"
    "vcpop.m t1,v0\n"
    "sd t1,0(%[R])\n"
    :: [A]"r"(A),[R]"r"(R),[vl]"r"(64UL) : "t0","t1","memory");
    report_e64("vcpop.m m8 vl=64", R, 1);
    VEND();
    return 0;
}
