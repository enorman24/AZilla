...
sim: provider-build
	...
	@bash $(RUN_SIM) \
	  ...
	  --snapshot "$(ELF)" \
	  --snapshot "$(TVM_LL)" --snapshot "$(TVM_COMPAT)" --snapshot "$(BUILD_MAIN_C)" \
	  --snapshot "$(APP_DIR)/manifest.json"; \
	rc=$$?; \
	$(TVM_MANIFEST_ENV) $(TVM_PYTHON) $(ARTIFACTS) sim-manifest \
	  ...
	  2>/dev/null || echo "flow: manifest emission skipped (artifacts.py / TVM env unavailable)"; \
	$(TVM_PYTHON) $(ARTIFACTS) ledger-append \
